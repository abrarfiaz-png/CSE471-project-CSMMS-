...
export default function MyServicesPage() {
  ...
  const canCreateService = user?.role === 'student' || user?.role === 'provider' || user?.role === 'admin'
  ...
  const addSlot = async e => {
    ...
    try {
      ...
      await loadSlotManagerData(showSlotForm)
    } catch (err) { toast.error(err.response?.data?.detail || 'Failed') }
  }
  ...
  const updateCapacity = async (serviceId, maxDailyCapacity) => {
    const val = parseInt(maxDailyCapacity)
    if (!Number.isFinite(val) || val < 1) return toast.error('Capacity must be at least 1')
    setSavingCapacityId(serviceId)
    try {
      await api.put(`/services/${serviceId}/capacity`, { max_daily_capacity: val })
  ...
  const loadSlotManagerData = async (serviceId) => {
    if (!serviceId) return
    setSlotLoading(true)
    try {
      const [slotsRes, blockedRes] = await Promise.all([
        api.get(`/services/${serviceId}/slots`),
        api.get(`/services/${serviceId}/blocked-dates`),
      ])
      setSlotList(slotsRes.data || [])
      setBlockedDates(blockedRes.data || [])
    } catch {
      toast.error('Failed to load slot manager data')
      setSlotList([])
      setBlockedDates([])
    } finally {
  ...
  const openSlotManager = async (serviceId) => {
    setShowSlotForm(serviceId)
    await loadSlotManagerData(serviceId)
  }

  const toggleSlotBlock = async (slot) => {
    try {
      await api.put(`/services/${showSlotForm}/slots/${slot.id}/block`, { is_blocked: !slot.is_blocked })
  ...
  const blockDate = async (e) => {
    e.preventDefault()
    if (!blockedDateForm.date) return toast.error('Select a date to block')
    try {
      await api.post(`/services/${showSlotForm}/blocked-dates`, blockedDateForm)
  ...
  const unblockDate = async (blockedDateId) => {
    try {
      await api.delete(`/services/${showSlotForm}/blocked-dates/${blockedDateId}`)
              ...
              <div key={s.id} className="card">
                ...
                {(user?.role === 'provider' || user?.role === 'admin') && (
                  <div className="mb-3">
                    ...
                    <label className="label mt-2">Daily Capacity</label>
                    <div className="flex gap-2">
                      <input
                        className="input"
                        type="number"
                        min="1"
                        value={s.max_daily_capacity || 1}
                        onChange={(e) => setServices(prev => prev.map(x => x.id === s.id ? { ...x, max_daily_capacity: e.target.value } : x))}
                      />
                      <button
                        className="btn-secondary"
                        onClick={() => updateCapacity(s.id, s.max_daily_capacity)}
                        disabled={savingCapacityId === s.id}
                      >
                        {savingCapacityId === s.id ? 'Saving…' : 'Save'}
                      </button>
                    </div>
                  </div>
                )}
                ...
                <div className="flex gap-2 flex-wrap">
                  {canManageSlots && (
                    <button onClick={() => openSlotManager(s.id)} className="btn-secondary text-xs py-1.5 flex items-center gap-1">
      ...
      {showSlotForm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-display font-bold text-xl">Manage Time Slots & Blocked Dates</h3>
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <form onSubmit={addSlot} className="space-y-3">
                <h4 className="font-semibold text-slate-700">Add Slot</h4>
              ...
              <form onSubmit={blockDate} className="space-y-3">
                <h4 className="font-semibold text-slate-700">Block Unavailable Date</h4>
                <div><label className="label">Date</label><input className="input" type="date" value={blockedDateForm.date} onChange={e => setBlockedDateForm(p => ({ ...p, date: e.target.value }))} required /></div>
                <div><label className="label">Reason (optional)</label><input className="input" value={blockedDateForm.reason} onChange={e => setBlockedDateForm(p => ({ ...p, reason: e.target.value }))} placeholder="Vacation, maintenance, etc." /></div>
                <button type="submit" className="btn-secondary w-full">Block Date</button>
              </form>
            </div>

            <div className="mt-5 grid md:grid-cols-2 gap-5">
              <div>
                <h4 className="font-semibold text-slate-700 mb-2">Existing Slots</h4>
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {slotLoading ? <p className="text-sm text-slate-400">Loading…</p> : slotList.length === 0 ? <p className="text-sm text-slate-400">No slots yet</p> : slotList.map(slot => (
                    <div key={slot.id} className="border border-slate-200 rounded-lg p-2 flex items-center justify-between">
                      <div className="text-sm">
                        <p className="font-medium">{slot.date}</p>
                        <p className="text-slate-500">{slot.start_time} - {slot.end_time}</p>
                      </div>
                      <button onClick={() => toggleSlotBlock(slot)} className={`text-xs px-2 py-1 rounded ${slot.is_blocked ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              ...
              <div>
                <h4 className="font-semibold text-slate-700 mb-2">Blocked Dates</h4>
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {slotLoading ? <p className="text-sm text-slate-400">Loading…</p> : blockedDates.length === 0 ? <p className="text-sm text-slate-400">No blocked dates</p> : blockedDates.map(d => (
                    <div key={d.id} className="border border-slate-200 rounded-lg p-2 flex items-center justify-between">
                      <div className="text-sm">
                        <p className="font-medium">{d.date}</p>
                        <p className="text-slate-500">{d.reason || 'No reason'}</p>
                      </div>
                      <button onClick={() => unblockDate(d.id)} className="text-xs px-2 py-1 rounded bg-slate-200 text-slate-700">Unblock</button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="flex justify-end mt-4">
              <button type="button" onClick={() => setShowSlotForm(null)} className="btn-primary">Done</button>
            </div>
          </div>
        </div>
      )}
